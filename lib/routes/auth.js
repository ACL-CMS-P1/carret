...
router
    ...
    .post('/signup', hasEmailAndPassword, checkUser, (req, res, next) => {
        ...
        User.exists({ email })
            ...
            .then(user => {
                // here is how you could wait for event to save
                return Promise.all([user, saveEvent(user, 'signup')]);
            })
                    ...
                    event.save()
                        .then(event => {
                            Event.checkFailedLogins({ email: `${event.email}` })
                                .then(res => {
                                    // change res to single boyd, not array.
                                    // where is this "return" going to do anything? noone listening
                                    if(res[0].tooManyAttempts === true) return failedLogins.lockUserAccount({ email: event.email });
                                });
                        });
                        // what if this promise chain fails?
                    throw next({
                        code: 401,
                        error: 'Invalid Login'
                    });
